...
COR_BMO_LINGUA     = (130, 100, 255) # Rosa fofo para a língua
...
def is_finger_open(tip_y, pip_y):
    ...
    return tip_y < pip_y - 0.02 # Threshold leve para compensar rotação

def detect_gestures_for_egg(hand_landmarks):
    """Detecta sinais de 'Jóia' e 'Rock' baseado nos landmarks."""
    # Pontas dos dedos
    ...
    # Lógica do ROCK (Indicador e Mínimo abertos, médio e anelar fechados, polegar aberto)
    if in_open and not mi_open and not an_open and pi_open and th_open:
        rock = True
...
def draw_bmo(frame, x, y, w_size=210, h_size=250, mood="neutral"):
    ...
    # 1. Corpo/Hardware do BMO (Ampliado se necessário)
    ...
    # Ajuste de proporção para os elementos internos
    scale = w_size / 210.0
    ...
    # Modo Super Feliz tem tela normal
    ...
    elif mood == "very_happy": # MODO ROCK ON com língua
        # Olhos normais e sorrisão aberto com LÍNGUA
        cv2.circle(frame, (cx - int(40*scale), cy - int(15*scale)), eye_size, COR_BMO_FACE, -1, lineType=cv2.LINE_AA)
        cv2.circle(frame, (cx + int(40*scale), cy - int(15*scale)), eye_size, COR_BMO_FACE, -1, lineType=cv2.LINE_AA)
        # Bocão aberto
        cv2.ellipse(frame, (cx, cy + int(10*scale)), (int(20*scale), int(16*scale)), 0, 0, 180, COR_BMO_FACE, -1, lineType=cv2.LINE_AA)
        # LÍNGUA ROSA (Elipse rosa fofa dentro da boca aberta)
        l_w = int(10 * scale)
        l_h = int(12 * scale)
        cv2.ellipse(frame, (cx, cy + int(20*scale)), (l_w, l_h), 0, 0, 360, COR_BMO_LINGUA, -1, lineType=cv2.LINE_AA)
        cv2.ellipse(frame, (cx, cy + int(20*scale)), (l_w, l_h), COR_BMO_FACE, 1, lineType=cv2.LINE_AA)

    elif mood == "super_happy": # MODO JÓIA FULLSCREEN
        # Olhos de arco (^ ^) super felizes
        arc_w = int(14 * scale)
        arc_h = int(12 * scale)
        cv2.ellipse(frame, (cx - int(40*scale), cy - int(10*scale)), (arc_w, arc_h), 0, 180, 360, COR_BMO_FACE, 4, lineType=cv2.LINE_AA)
        cv2.ellipse(frame, (cx + int(40*scale), cy - int(10*scale)), (arc_w, arc_h), 0, 180, 360, COR_BMO_FACE, 4, lineType=cv2.LINE_AA)
        # Bochechas ultra rosadas e brilhantes
        cv2.circle(frame, (cx - int(55*scale), cy + int(15*scale)), int(10*scale), COR_BMO_BOCHECHA, -1, lineType=cv2.LINE_AA)
        cv2.circle(frame, (cx + int(55*scale), cy + int(15*scale)), int(10*scale), COR_BMO_BOCHECHA, -1, lineType=cv2.LINE_AA)
        # Bocão aberto feliz (Tamanho total)
        cv2.ellipse(frame, (cx, cy + int(10*scale)), (int(22*scale), int(18*scale)), 0, 0, 180, COR_BMO_FACE, -1, lineType=cv2.LINE_AA)
...
while True:
    ...
    # Reiniciar flags de ovo a cada frame
    joinha_detected = False
    rock_detected = False

    if results.multi_hand_landmarks:
        # 1. DETECÇÃO DE EASTER EGGS (Prioridade Visual)
        for hand_landmarks in results.multi_hand_landmarks:
            joinha, rock = detect_gestures_for_egg(hand_landmarks)
            if joinha: joinha_detected = True
            if rock: rock_detected = True

        # 2. SE NENHUM EASTER EGG TIVER PRIORIDADE, PROCESSA O SIMULADOR NORMALMENTE
        # (Modo Jóia Fullscreen interrompe a simulação e HUD)
    ...
    if exploded_state:
        ...
        # Definir humor normal baseado na Pressão
        if pressure < 120:
            bot_mood = "happy"
        elif pressure < 280:
            bot_mood = "neutral"
        ...
        # Prioridade do EASTER EGG ROCK (Mostra língua) sobre o humor normal
        # (Jóia e Explosão têm prioridades ainda maiores e são tratadas na renderização)
        if rock_detected:
            bot_mood = "very_happy" # Face com Língua
    ...
    # ==========================
    # RENDERIZAÇÃO FINAL (MÁQUINA DE DESENHO)
    # ==========================
    
    # --- PRIORIDADE MÁXIMA: EASTER EGG JÓIA (Fullscreen) ---
    if joinha_detected:
        draw_hud_panel(frame, 0, 0, w, h, alpha=0.9) # Escurecer fundo
        # Desenhar BMO centralizado e Gigante (3x tamanho normal)
        scale_joinha = 3.0
        bw = int(210 * scale_joinha)
        bh = int(250 * scale_joinha)
        cx = (w // 2) - (bw // 2)
        cy = (h // 2) - (bh // 2)
        # Desenha com mood SUPER_HAPPY (Olhos ^ ^ e Bochechas rosadas)
        draw_bmo(frame, cx, cy, w_size=bw, h_size=bh, mood="super_happy")
        cv2.putText(frame, "EASTER EGG DETECTADO!", (cx + 50, bh - 100), cv2.FONT_HERSHEY_SIMPLEX, 1.2, COR_SUCESSO, 4, cv2.LINE_AA)
    ...
    # --- RENDERIZAÇÃO NORMAL (Simulador) ---
        ...
        if exploded_state: cv2.putText(frame, "EXPLODIU!", (w // 2 - 130, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (50, 50, 255), 3)
        else:
        ...
        if not results.multi_hand_landmarks:
            ...
            cv2.putText(frame, "INSIRA AS MAOS PARA CONTROLAR", (w//2 - 215, h - 42), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (100, 200, 255), 2)

        # 3. Desenhar BMO Canto Superior (Com mood definido acima, incluindo rock)
        draw_bmo(frame, w - 260, 40, mood=bot_mood)

    # Exibir o frame final
    cv2.imshow(NOME_JANELA, frame)
